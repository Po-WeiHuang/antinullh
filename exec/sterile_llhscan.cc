              ...
              const std::string &oscGridConfigFile_)
{
  ...
  std::vector<NormFittingStatus>* fitstat;
  fitstat->push_back(FALSE);
  lh.AddPdfs(pdfs, pdfGroups, genRates,fitstat);
  ...
  for (ParameterDict::iterator it = mins.begin(); it != mins.end(); ++it)
  {
    ...
    for (int i = 0; i < npoints; i++)
    {
      ...
      //std::cout<<"Ready to Evaluate"<<std::endl;
      double llh = lh.Evaluate();
      //std::cout<<"Finish Evaluate at "<< i <<std::endl;
      std::cout<<"llh "<< llh <<" parval "<<parval<<std::endl;
